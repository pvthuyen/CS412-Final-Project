...
struct InvertedIndex {
    ...
    void buildTfidf() {
        ...
        for (int i = 0; i < nWords; i++) {
            ...
            for (int j = 0; j < index[i].size(); j++) {
                double tf = sqrt(fabs(frequency[i][j] / sumFrequency[index[i][j]]));
    ...
    vector<double> makeQueryTfidf(arma::vec freq, arma::uvec termId) {
    ...
    void add(const arma::vec &freq, const arma::uvec &termId, int docId) {
        nDocs++;
